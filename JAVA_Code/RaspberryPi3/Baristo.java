 ...
 */
 public static void main (String[] args){
   ...
   errorCodes = new int[MAX_ERROR_CODES];   //Start configuration with no errors                  

   System.out.println("Baristo kiosk starting up.");
   inputArguments = EngineeringTerminal.GetUserInput();
   errorCodes[0] = CongfigureBaristo(Integer.parseInt(inputArguments[0]), Double.parseDouble(inputArguments[1]));
 ...
 private static int CongfigureBaristo(int bootMode, double verNum){
   ...
   //Critical Subsystems
   errorCodes[1] = kioskPLC.IntializeBatteries(bootMode, verNum);
   errorCodes[2] = kioskPLC.IntializeSolarPanels(bootMode, verNum);
   parrallaxPartNumber = 999; //TO-DO DETERMINE RFID PART NUMBER
   errorCodes[3] = kioskRFID.IntializeRFID(bootMode, verNum, ParallaxHardware.RFID_TX_RX_PN28140);
